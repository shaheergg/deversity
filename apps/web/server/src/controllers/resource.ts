...
export const getResourcesForCourse = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send("An error occurred while fetching resources for course");
  }
};
...
export const getResourcesForModule = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send("An error occurred while fetching resources for module");
  }
};
...
export const addResourceToModule = async (req, res) => {
  try {
    const { moduleId } = req.params;
    const { title, description = "null", url, type } = req.body;
    ...
    res
      .status(500)
      .send("An error occurred while adding a resource to the module");
  }
};
...
export const addResourceToCourse = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send("An error occurred while adding a resource to the course");
  }
};
